...
CREATE TYPE "public"."cylinder_status" AS ENUM('instalado', 'en_planta', 'pendiente_reinstalacion', 'reinstalado', 'condenado');--> statement-breakpoint
CREATE TYPE "public"."document_type" AS ENUM('cedula', 'carnet');--> statement-breakpoint
CREATE TYPE "public"."inspection_status" AS ENUM('inspeccion_inicial', 'recalificacion', 'por_programar', 'cita', 'certificado');--> statement-breakpoint
...
CREATE TABLE "gnc_cylinders" (
	...
	"status" "cylinder_status" DEFAULT 'instalado',
...
CREATE TABLE "vehicles" (
	...
	"created_at" timestamp DEFAULT now(),
	"updated_at" timestamp DEFAULT now(),
	CONSTRAINT "vehicles_codigo_unico_gnc_unique" UNIQUE("codigo_unico_gnc"),
...
CREATE UNIQUE INDEX "idx_vehicle_docs_minio_key" ON "vehicle_documents" USING btree ("minio_key");--> statement-breakpoint
